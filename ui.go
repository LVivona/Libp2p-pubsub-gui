...
func LoginUI() {
	...
	form := tview.NewForm().AddInputField("nickname", "", 30, nil, nil).AddInputField("Chat Room", "", 30, nil, nil)
	form.AddButton("Enter", func() {
		app.Stop()
		nick := form.GetFormItem(0).(*tview.InputField).GetText()
		room := form.GetFormItem(1).(*tview.InputField).GetText()
...
/*
	...
	form := tview.NewForm().AddInputField("nickname", "", 30, nil, nil).AddInputField("Chat Room", "", 30, nil, nil)
	form.AddButton("Enter", func() {
		nickname := form.GetFormItem(0).(*tview.InputField).GetText()
		room := form.GetFormItem(1).(*tview.InputField).GetText()
		enterRoom(app, nickname, room)
	}).
		AddButton("Quit", func() {
			app.Stop()
		})
